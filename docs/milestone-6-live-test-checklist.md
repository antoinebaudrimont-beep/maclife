...
- Each rule lists exact accepted window and process identities, an optional family, a quit action, and whether the process may be shared.
...
- Every adapter action requires live same-user `_NET_WM_PID` validation, expected process identity, one unambiguous PID across its meaningful windows, and a fresh unchanged process/window revalidation immediately before action.
- Adapter validation failure refuses. It never falls through to generic SIGTERM.
- Compatibility quit uses native `WM_DELETE_WINDOW`, preserving application confirmation behavior. No compatibility adapter sends SIGTERM.
...
- 58 tests pass with `RUSTFLAGS="-D warnings" cargo test --all-targets` before the final documentation-only change.
